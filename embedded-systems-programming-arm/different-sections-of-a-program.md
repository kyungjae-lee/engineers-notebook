...
* main.o (Object file in ELF format)
  ...
  * **Other special sections**
    ...
    When ELF file is loaded onto microcontroller, these sections are placed in ROM.
...
* Summary
  ...
  <img src="./img/data-and-sections-summary.png" alt="transfer-of-data-section-to-ram" width="900">

  [!] Note: `.data` section is associated with two different addresses; LMA, VMA. LMA is the address when the `.data` section is still on the FLASH, and VMA is the address when it is copied to the RAM (a.k.a. run-time address).

  [!] Note: In microcontroller, nothing is virtual. Everything is physical. So, do not regard VMA as the "Virtual memory" generated by the Memory Management Unit (MMU). 

  [!] Note: Global const data do not need to consume RAM since they won't get updated during the run-time. So, no VMA or run-time address.
